 ...
 * --text-caption 字阶,720 阅读宽度上限居中(design 基线 §6.4 第 1 条,规格值非样式取值)。
 *
...
export function Disclaimer({ className, ...props }: ComponentProps<"footer">) {
  return (
    <footer
      className={cn("mx-auto w-full max-w-2xl border-t border-hairline pt-base-lg", className)}
      {...props}
    >
      <p className="text-caption text-ink-mute">{DISCLAIMER_FULL}</p>
    </footer>
  );
}
