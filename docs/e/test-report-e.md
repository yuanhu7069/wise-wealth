...
> 状态:**草稿——待苑问人工走查后定稿**(§4 走查记录 + §6 终验)
> 自动门禁最终状态:2026-09-23 `scripts/check.sh` 五步全绿(clippy -D warnings / cargo test 134 / biome / tsc / Token 门禁)
...
- [ ] 登录 → P01 追踪卡两态(未打卡提示/已打卡摘要)→ 去打卡 → P05
- [ ] P05 五段走查(录入/进度/偏离/历史/导出)× 亮/暗 × 375px
- [ ] 录入:预览千分位 / 覆盖确认 / 本月特殊 / 提交失败输入保留
- [ ] 导出:双 CSV 下载(文件名/BOM/Excel 中文)
- [ ] 删除最新月(二次确认)与历史月不可删
- [ ] **DoD 验收动作**:本月真实数字打卡 1 条(次月复访完成第 2 条后全闭)
...
- cargo audit / npm audit:票 06 收口前复跑,结果回填
- 已知挂账:rsa 依赖 medium(B 期 I-7,上游无修复,继续挂);TLS RISK-E-1(2026-09-22 拍板自用期接受,对外前必须解决)

## 8. 已知偏离与遗留

- 工具链:`gen-types.sh` 在 TS7 环境崩溃(TS7 无 ts.factory),本次以 vendor ts5.9.3 隔离环境生成;**长期修法待苑问拍板**(工单 02 Comments)
- I-8 收入不足文案歧义、I-12 非 Chromium 走查:继续挂账(非本期域)
- 代码评审(code-review):首次运行被 API 限额打断,重跑结果回填 §9
